...
- **垃圾清理**：删除缓存、临时文件、缩略图、日志、残留数据。理论上不会影响用户主动存储的数据，但**仍可能因系统/ROM 差异导致误删**
- **应用冻结**：通过 Shizuku `pm suspend` 可逆操作。但**部分 ROM 厂商实现差异**可能导致冻结后无法恢复（极小概率）
- **一键优化**：自动清理垃圾 + 内存提示。**不会**删除用户照片、文档、聊天记录
- **电池卫士**：仅展示数据，不修改系统电源策略
- **存储分析**：仅读取路径元数据，不上传任何数据
