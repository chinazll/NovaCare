...
import com.novacare.feature.appdetail.AppDetailScreen
...
import com.novacare.feature.export.ExportScreen
...
import com.novacare.feature.oneclick.OneClickScreen
...
object Routes {
    ...
    // ---- v0.21.0 新增：app 详情 / 一键体检 / 数据导出 ----
    // APP_DETAIL 携带 packageName 路径参数；ONE_CLICK 与 EXPORT 是无参路由。
    const val APP_DETAIL = "appdetail/{packageName}"
    const val ONE_CLICK = "oneclick"
    const val EXPORT = "export"

    /** 构造 appdetail 路由 —— 强制要求 packageName（避免被 null 包名绕过） */
    fun appDetail(packageName: String): String = "appdetail/$packageName"
}
        ...
        NavHost(
            ...
            composable(Routes.HOME) {
                ...
                HomeScreen(
                    ...
                    onOpenScreenTime = { navController.navigateTo(Routes.SCREENTIME) },
                    onOpenTraffic = { navController.navigateTo(Routes.TRAFFIC) },
            ...
            // ---- v0.21.0 新增：应用详情 / 一键体检 / 数据导出 ----
            // APP_DETAIL 是压栈下钻：返回键回上一屏（通常是首页或守护中心）。
            // ONE_CLICK 是首页"一键体检"的入口；完成后的动作按钮会切到 CLEAN/FREEZE tab。
            // EXPORT 是设置页里的导出入口，返回键回设置。
            composable(Routes.APP_DETAIL) { entry ->
                // 路径参数已由 SavedStateHandle 注入 ViewModel；这里只取 onBack
                AppDetailScreen(onBack = { navController.popBackStack() })
            }
            composable(Routes.ONE_CLICK) {
                OneClickScreen(
                    onOpenClean = { navController.switchTab(Routes.CLEAN) },
                    onOpenFreeze = { navController.switchTab(Routes.FREEZE) },
                    onBack = { navController.popBackStack() },
                )
            }
